...
quadtree_t *quadtree_create( terrain_tile_t *tile, const bool list_nodes, quadtree_t *quadtree ) {
	...
	const float size_in_memory = (float)(quadtree->node_count*(sizeof(node_t)+sizeof(aabb_t))+sizeof(quadtree_t));
	snprintf( msg, MAX_LEN_MESSAGES-1,
			"Quadtree created. %d nodes, size in memory %fkb, %d*%d top level nodes",
