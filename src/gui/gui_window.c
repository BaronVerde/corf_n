...
gui_window_t* gui_window_create(
		const char *const title, const font_info_t *const font,
		const int upper_left_x, const int upper_left_y,
		const float app_window_size_x, const float app_window_size_y,
	...
	if( NULL != w ) {
		...
		if( NULL == w->internals ) {
			...
			if( !glIsProgram( shader_program ) )
				sp_create( "src/gui/glyph_shader.vs", "src/gui/glyph_shader.fs", &shader_program );
			...
			w->scissor_size_x = width;
			w->scissor_size_y = height;
		}
	}
	return w;
}

bool gui_window_begin( const gui_window_t *const w ) {
	// @todo: projection matrix must be renewed when app. window size changes
	mat4f projection;
	mat4f_ortho( 0.0f, w->app_window_size_x, 0.0f, w->app_window_size_y, 0.0f, 1.0f, &projection );
...
bool gui_window_add_static_text( gui_window_t* w, const char *const text, const float pos_x, const float pos_y ) {
	...
	for( int i = 0; i < in->num_dynamic_elements; ++i ) {
		...
		switch( e->datatype ) {
			...
			case gui_int:
				...
				break;
			case gui_unsigned_int:
				snprintf( &to_display[0], MAX_GUI_ELEMENT_LENGTH, "%9u", *(unsigned int*)e->variable );
				break;
			default:
...
// @todo centralize this together with others
static const GLuint VERTEX_BUFFER_BINDING_INDEX = 0;
...
void gui_window_render( gui_window_t* w, const vec3f* color ) {
	...
	glScissor(
			w->upper_left_x, (int)w->upper_left_y-w->scissor_size_y,
			w->scissor_size_x, w->scissor_size_y
	);
	glEnable( GL_BLEND );
	...
	glVertexArrayVertexBuffer(
			i->vertex_array, VERTEX_BUFFER_BINDING_INDEX, i->static_vertex_buffer, 0, sizeof( vec4f )
	);
	glDrawArrays( GL_TRIANGLES, 0, i->num_static_vertices );
	glVertexArrayVertexBuffer(
			i->vertex_array, VERTEX_BUFFER_BINDING_INDEX, i->dynamic_vertex_buffer, 0, sizeof( vec4f )
	);
...
static bool glyph_screen_coords(
	...
	for( const char* p = text; *p; ++p ) {
		...
		const vec4f b1 = { x2,				y2 + g->size_y,	x_min, y_min }; buffer[(*index)++] = b1;
		const vec4f b2 = { x2,				y2,				x_min, y_max }; buffer[(*index)++] = b2;
		const vec4f b3 = { x2 + g->size_x,	y2,				x_max, y_max }; buffer[(*index)++] = b3;
		const vec4f b4 = { x2,				y2 + g->size_y,	x_min, y_min }; buffer[(*index)++] = b4;
		const vec4f b5 = { x2 + g->size_x,	y2,				x_max, y_max }; buffer[(*index)++] = b5;
		const vec4f b6 = { x2 + g->size_x,	y2 + g->size_y,	x_max, y_min }; buffer[(*index)++] = b6;
	}
	return true;
}
