...
font_info_t* font_create( const char *const filename, const int height ) {
	if( font_init_and_check( filename ) ) {
		...
		if( NULL != font_info ) {
			...
			if( height < 6 || height > 36 )
				logbook_log( LOG_WARNING, "Font height should be [6-36], for now" );
			// @todo check return code 0 == success, use unsigned int for height
			FT_Set_Pixel_Sizes( face, 0, (FT_UInt)font_info->height );
			...
			snprintf( msg, MAX_LEN_MESSAGES, "Loading font '%s'", filename );
			...
			for( GLubyte i = 32; i < 128; ++i ) {
				if( FT_Load_Char( face, i, FT_LOAD_RENDER ) ) {
					snprintf( msg, MAX_LEN_MESSAGES, "Failed to load glyph #%d, char '%c'", i, i );
					logbook_log( LOG_WARNING, msg );
					continue;
				}
				int idx = i - 32;
