...
inline void camera_create( const vec3f *const position, const vec3f *const target ) {
...
inline bool camera_mouse_move( float x, float y ) {
	bool handled = false;
	float xf = x, yf = y;
...
inline void camera_print_position() {
	...
	sprintf( msg, "Cam position: %.2lf/%.2lf/%.2lf; target: %.2lf/%.2lf/%.2lf; mode: %s",
...
inline vec3f *camera_get_front() {
	return &camera.front;
}

inline vec3f *camera_get_up() {
	return &camera.up;
}
...
inline mat4f *camera_get_view_matrix() {
...
inline mat4f *camera_get_projection_matrix() {
...
inline void camera_calculate_initial_angles() {
	...
	camera.yaw = (float)degrees( atan2( -direction.x, direction.z ) );
...
inline void camera_update_vectors() {
	if( ORBITING == camera.mode ) {
		...
		const float yaw = radians(camera.pitch);
		const float pitch = radians(camera.yaw);
		...
		// set view matrix and update frustum vectors @float conversions
		vec3f posf = { (float)camera.position.x,(float)camera.position.y,(float)camera.position.z };
		vec3f frontf = { (float)front.x,(float)front.y,(float)front.z };
		vec3f upf = { (float)camera.up.x,(float)camera.up.y,(float)camera.up.z };
		mat4f_lookat( &posf, &frontf, &upf, &camera.view_matrix );
		frustum_set_camera_vectors(
